...
def df_filter(main_df):
    ...
    filtered_df = main_df[(main_df['MSNDATE'] >= start_date) & (main_df['MSNDATE'] <= end_date)]
    grouped_df = filtered_df.groupby('COUNTRY_FLYING_MISSION')[['AC_ATTACKING']].sum().reset_index()
    return grouped_df
